...
string BTrackVamp::getIdentifier() const { return "btrack-vamp"; }

string BTrackVamp::getName() const { return "BTrack"; }

string BTrackVamp::getDescription() const {
  // Return something helpful here!
  return "A Real-Time Beat Tracker";
}

string BTrackVamp::getMaker() const {
  // Your name here
  return "Adam Stark, Matthew Davies and Mark Plumbley";
}

int BTrackVamp::getPluginVersion() const {
  // Increment this each time you release a version that behaves
  // differently from the previous one
  return 1;
}

string BTrackVamp::getCopyright() const {
...
BTrackVamp::InputDomain BTrackVamp::getInputDomain() const {
  return TimeDomain;
}

size_t BTrackVamp::getPreferredBlockSize() const {
  return 1024; // 0 means "I can handle any block size"
}

size_t BTrackVamp::getPreferredStepSize() const {
  return 512; // 0 means "anything sensible"; in practice this
              // means the same as the block size for TimeDomain
              // plugins, or half of it for FrequencyDomain plugins
}

size_t BTrackVamp::getMinChannelCount() const { return 1; }

size_t BTrackVamp::getMaxChannelCount() const { return 1; }

BTrackVamp::ParameterList BTrackVamp::getParameterDescriptors() const {
  ParameterList list;
...
float BTrackVamp::getParameter(string identifier) const {
...
void BTrackVamp::setParameter(string identifier, float value) {
  if (identifier == "parameter") {
    // set the actual value of your parameter
  }
}

BTrackVamp::ProgramList BTrackVamp::getPrograms() const {
  ProgramList list;
...
string BTrackVamp::getCurrentProgram() const {
  return ""; // no programs
}

void BTrackVamp::selectProgram(string name) {}

BTrackVamp::OutputList BTrackVamp::getOutputDescriptors() const {
  OutputList list;
...
bool BTrackVamp::initialise(size_t channels, size_t stepSize,
                            size_t blockSize) {
...
BTrackVamp::FeatureSet BTrackVamp::process(const float *const *inputBuffers,
  ...
  RealArrayBuffer::Ptr btrack_input_buffer =
      std::make_shared<RealArrayBuffer>(m_blockSize);
  std::vector<double> &frame = btrack_input_buffer->data();
  ...
  if (b->beat_due_in_current_frame()) {
    ...
    beat.timestamp = timestamp - Vamp::RealTime::frame2RealTime(
...
BTrackVamp::FeatureSet BTrackVamp::getRemainingFeatures() {
  return FeatureSet();
}
