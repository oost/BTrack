...
#include "../../src/onset_detection_function.h"
...
void btrack_dsp64(t_btrack *x, t_object *dsp64, short *count, double samplerate,
                  long maxvectorsize, long flags);
t_int *btrack_perform(t_int *w);
void btrack_perform64(t_btrack *x, t_object *dsp64, double **ins, long numins,
...
int C74_EXPORT main(void) {
  //--------------------------------------------------------------
  t_class *c = class_new("btrack~", (method)btrack_new, (method)btrack_free,
                         (long)sizeof(t_btrack), 0L, A_GIMME, 0);

  //--------------------------------------------------------------
  class_addmethod(c, (method)btrack_float, "float", A_FLOAT, 0);
  class_addmethod(
      c, (method)btrack_dsp, "dsp", A_CANT,
      0); // Old 32-bit MSP dsp chain compilation for Max 5 and earlier
  class_addmethod(c, (method)btrack_dsp64, "dsp64", A_CANT,
                  0); // New 64-bit MSP dsp chain compilation for Max 6
...
void *btrack_new(t_symbol *s, long argc, t_atom *argv) {
  ...
  if (x) {
    dsp_setup((t_pxobject *)x, 1); // MSP inlets: arg is # of inlets and is
...
// for the signal chain in Max 5 and earlier. In this case we register the
// 32-bit, "btrack_perform" method.
void btrack_dsp(t_btrack *x, t_signal **sp, short *count) {
...
//===========================================================================
// this is the Max 6 version of the dsp method -- it registers a function for
// the signal chain in Max 6, which operates on 64-bit audio signals.
void btrack_dsp64(t_btrack *x, t_object *dsp64, short *count, double samplerate,
                  long maxvectorsize, long flags) {
  // get hop size and frame size
...
void btrack_perform64(t_btrack *x, t_object *dsp64, double **ins, long numins,
                      double **outs, long numouts, long sampleframes,
                      long flags, void *userparam) {
  t_double *inL = ins[0]; // we get audio for each inlet of the object from the
                          // **ins argument
  int n = sampleframes;
...
//===========================================================================
void btrack_on(t_btrack *x) { x->should_output_beats = true; }
